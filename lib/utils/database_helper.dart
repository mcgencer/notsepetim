...
import 'package:flutter_app/models/notlar.dart';
...
// open the database
  ...
  Future<int> kategoriGuncelle(Kategori kategori) async {
    ...
    var sonuc = await db.update(
        "kategori", kategori.toMap(), where: 'kategoriID = ?',
  ...
  Future<int> kategoriSil(int kategoriID) async {
    ...
    var sonuc = await db.delete(
        "kategori", where: 'kategoriID = ?', whereArgs: [kategoriID]);
    return sonuc;
  }
  ...
  Future<int> notGuncelle(Not not) async {
    var db = await _getDatabase();
    var sonuc = await db.update(
        "not", not.toMap(), where: 'notID = ?', whereArgs: [not.notID]);
    return sonuc;
  }
